...
function Replace-UploadMarkdownWithPlaceholders {
...
function Replace-PlaceholdersWithInlineImages {
...
function New-AdoWorkItemInitial {
    ...
    $cleanMarkdown = Replace-UploadMarkdownWithPlaceholders -Markdown $descriptionRaw -Uploads $uploads
...
function Finalize-AdoWorkItemContent {
    ...
    foreach ($upload in $Uploads) {
        try {
            ...
            $adoAttachment = Upload-AdoAttachment -FilePath $destPath
    ...
    try {
        $markdownWithPlaceholders = Replace-UploadMarkdownWithPlaceholders -Markdown $OriginalMarkdown -Uploads $Uploads
        $markdownWithInlineImages = Replace-PlaceholdersWithInlineImages -Markdown $markdownWithPlaceholders -AttachmentUrlMap $attachmentUrlMap
...
function Migrate-GitLabIssueLinksToAdo {
...
try {
    ...
    foreach ($issue in $issues) {
        ...
        $finalizationResult = Finalize-AdoWorkItemContent -WorkItemId $adoId -GitLabIssue $issue -Uploads $initialItem.uploads -OriginalMarkdown $initialItem.originalMarkdown
    ...
    if ($Config.MigrateIssueLinks) {
        ...
        $migratedLinkCount = Migrate-GitLabIssueLinksToAdo -GitLabIssues $issues -IssueMapByIid $issueMapByIid
    }
